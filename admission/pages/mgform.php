...
$sql_query = "SELECT form_owner.regnum as regid, form_owner.admission_status, form_owner.status,  form_owner.rectime, form_bio.regnum, form_bio.firstname, form_bio.othername, form_bio.surname, form_bio.passport, form_prog.regnum, form_prog.progid, programmes.course, programmes.mode ,programmes.program FROM form_owner 
...
where admission_status = 0 and `status` = 11 ORDER BY rectime DESC" ;
...
echo '
                                        <tr>
                                            ...
                                            <td><img src="../../index/backend/image/'.$row['passport'].'"  width="100" height="100" alt="profile-image"></td>
                                            <td>'.$row["regid"].'</td>
                                            <td>'.$row["firstname"]." ".$row["othername"]." ".$row["surname"].'</td>
                                            <td ><center><p>'.strtoupper($row["course"]).'<br>'.strtoupper($row["program"]).'<br>'.strtoupper($row["mode"]).'</p></center></td>
                                           <td>'.$button.'</button> 
                                        </tr>';
                                        ?>
                                        <?php }}?>
